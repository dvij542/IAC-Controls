...
def mpcCallback():
    ...
    xs=[x_p.poses[goal_point_index].pose.position.x,x_p.poses[goal_point_index].pose.position.y,goal_point_yaw]
    ...
    #while(sqrt(((x_bot - xs[0])**2 + (y_bot - xs[1])**2)+(yaw_car-xs[2])**2)>.1):
        ...
        collecting_data=first_predicted_state- reshape(current_pose,1,3)

        print("collecting_data",np.linalg.pinv(collecting_data))
        ...
        #print('predicted_trajectory',predicted_trajectory)	
        	...
        	#print('x_coordinate',ff_value[k:,0])

        #while not rospy.is_shutdown():

        #predicted_trajectory.header.stamp=rospy.get_rostime()
