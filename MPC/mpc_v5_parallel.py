...
file = NULL  # File to read the global reference line, if NULL then centre line will be taken
...
def mpcCallback(trajectory_to_follow, angle_heading, curr_pos, curve, steering, speed, goaltheta, all_vehicles, roadwidth):
    x_bot = 0
    y_bot = 0
    ...
    if trajectory_to_follow!=NULL:
    	mini = 0
    	minval = 10000
    	for i in range(k):
    		if distance(trajectory_to_follow[i,0], trajectory_to_follow[i,1], curr_pos[0], curr_pos[1]) < minval:
    			minval = distance(trajectory_to_follow[i,0], trajectory_to_follow[i,1], curr_pos[0], curr_pos[1])
    			mini = i
    	points = [trajectory_to_follow[i,:]-curr_pos,trajectory_to_follow[(i+1)%k,:]-curr_pos,trajectory_to_follow[(i+2)%k,:]-curr_pos,trajectory_to_follow[(i+3)%k,:]-curr_pos]
    	points = np.array(points)
    	tr_matrix = np.array([[cos(angle_heading),-sin(angle_heading)],[sin(angle_heading),cos(angle_heading)]])
    	points = np.mul(points,tr_matrix)
    	M = np.array([points**0,points , points**2, points**3]).T
    	C = np.mul(np.inverse(M),points[:,1:])
...
with rti.open_connector(
    ...
    if file != NULL:
    	trajectory_to_follow = np.loadtext(file)
    while True:
        input_radar_F.wait()
        input_radar_F.take()
        no_of_vehicles = 0
        ...
        for sample in input.samples.valid_data_iter:
            ...
            print("Curve : ", curve)
            curr_steering_array, target_speed_array = (mpcCallback(trajectory_to_follow, np.array([px,py]), angle_heading, curve, curr_steering, curr_speed, 0, all_vehicles, roadwidth))
