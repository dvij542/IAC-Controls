...
P=SX.sym('P',10+4*max_no_of_vehicles+8)
...
for k in range(0,N,1):
    ...
    distance_l =  ((st[0]-itr[no_iters-1,k])**2 + (st[1]-F_val[0,k])**2)**(1/2)
    ...
    distance_r =  ((st[0]-itr_r[no_iters-1,k])**2 + (st[1]-F_val_r[0,k])**2)**(1/2)
    ...
    pen[0,k] =  distance_l-4.5
    pen[1,k] =  distance_l+4.5
...
def mpcCallback(trajectory_to_follow, curr_pos, angle_heading, curve, curve_l, curve_r, steering, speed, goaltheta, all_vehicles, roadwidth):
    ...
    p=current_pose+curve+current_control+curve_l+curve_r
...
with rti.open_connector(
    ...
    print(traj_followed)
    plt.plot(traj_followed[0],traj_followed[1],'ro')
    plt.plot(trajectory_to_follow[0]/100.0,trajectory_to_follow[1]/100.0,'ro')
